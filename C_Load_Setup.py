...
class Ui_C_Load_Setup(object):

    def setupUi(self, C_Load_Setup):
        C_Load_Setup.setObjectName("C_Load_Setup")
        C_Load_Setup.resize(313, 364)
        self.centralwidget = QtWidgets.QWidget(C_Load_Setup)
        ...
        C_Load_Setup.setCentralWidget(self.centralwidget)
        self.statusbar = QtWidgets.QStatusBar(C_Load_Setup)
        self.statusbar.setObjectName("statusbar")
        C_Load_Setup.setStatusBar(self.statusbar)

        self.retranslateUi(C_Load_Setup)
        QtCore.QMetaObject.connectSlotsByName(C_Load_Setup)

    def retranslateUi(self, C_Load_Setup):
        _translate = QtCore.QCoreApplication.translate
        C_Load_Setup.setWindowTitle(_translate("C_Load_Setup", "C Load"))
        self.groupBox.setTitle(_translate("C_Load_Setup", "Range"))
        self.groupBox_2.setTitle(_translate("C_Load_Setup", "Step size"))
        self.groupBox_3.setTitle(_translate("C_Load_Setup", "Start value"))
    
    def get_min_value(self):
        text = self.min_value_edit.text()
        Ui_C_Load_Setup.min_value = text
    
    def get_max_value(self):
        text = self.max_value_edit.text()
        Ui_C_Load_Setup.max_value = text

    def get_step_size(self):
        text = self.step_size_edit.text()
        Ui_C_Load_Setup.step_size = text    

    def get_start_value(self):
        text = self.start_value_edit.text()
        Ui_C_Load_Setup.start_value = text

if __name__ == "__main__":
    app = QtWidgets.QApplication(sys.argv)
    C_Load_Setup = QtWidgets.QMainWindow()
    ui = Ui_C_Load_Setup()
    ui.setupUi(C_Load_Setup)
    C_Load_Setup.show()
    sys.exit(app.exec_())